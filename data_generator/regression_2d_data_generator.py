...
'''
    逻辑：对于训练样本的组织逻辑，是这样的---待回归的训练数据，所有的图像文件会放到同一文件夹中，然后需要创建一个训练数据对应的label.csv文件，里面包含了每个图像文件对应的groundtruth，这样的csv文件一般有两个，一个训练用，一个验证用。所以，一般会创建两个DataGenerator对象{train_generator, validation_generator},然后分别使用parse_csv函数创建。
'''
class DataGenerator:
    '''
    ...
    def __init__(self,
        ...
        self.labels_format = {'x1': labels_output_format.index('x1'),
    ...
    def parse_csv_for_regression(self,
                                 images_dir,
                                 labels_filename,
                                 input_format,
                                 random_sample=False,
