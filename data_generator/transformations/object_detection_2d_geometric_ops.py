...
########################################
#注意，这里的label都是绝对坐标.
#结论：在准备训练数据的阶段，最好使用绝对坐标作为target ground truth，然后专门使用encoder来对转化之后的绝对坐标进行编码，例如转为相对坐标.
########################################
...
from data_generator.transformations.object_detection_2d_image_boxes_validation_utils import BoxFilter, ImageValidator
...
class Resize:
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        if labels is None:
            ...
            labels = np.copy(labels)
            # 下面的两句话表明了这里所说的labels一定是绝对坐标，而非是相对坐标.
...
class Resize_arbitrary:
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        xmin = self.labels_format['x1']
        ymin = self.labels_format['y1']
        ...
        if labels is None:
            ...
            labels = np.copy(labels)
            # 下面的两句话表明了这里所说的labels一定是绝对坐标，而非是相对坐标.
            # 将原图中的绝对位置数值转为resize以后的图中的绝对位置信息.
            labels[:, [ymin]] = np.round(labels[:, [ymin, ]] * (self.out_height / img_height), decimals=0)
            labels[:, [xmin]] = np.round(labels[:, [xmin, ]] * (self.out_width / img_width), decimals=0)
...
class ResizeRandomInterp_arbitrary:
    '''
    Resizes images to a specified height and width in pixels using a radnomly
    selected interpolation mode.
    '''
    ...
    def __init__(self,
        ...
        self.box_filter = box_filter
        self.labels_format = labels_format
        self.resize = Resize_arbitrary(height=self.height,
                             width=self.width,
                             box_filter=self.box_filter,
                             labels_format=self.labels_format)
...
class Flip_arbitrary(Flip):
    def __init__(self, dim='horizontal', labels_format={'x1': 0, 'y1': 1}):
        super().__init__(dim, labels_format)

    def __call__(self, image, labels=None, return_inverter=False):
        img_height, img_width = image.shape[:2]

        x1 = self.labels_format['x1']
        y1 = self.labels_format['y1']
        ...
        if self.dim == 'horizontal':
            ...
            if labels is None:
                ...
                labels = np.copy(labels)
                labels[:, [x1, ]] = img_width - labels[:, [x1, ]]    # 注意，这里只用宽度直接减去label，表明label是绝对坐标，而非相对坐标.
                return image, labels
        else:
            image = image[::-1]
            if labels is None:
                ...
                labels = np.copy(labels)
                labels[:, [y1, ]] = img_height - labels[:, [y1, ]]
                return image, labels
...
class RandomFlip_arbitrary:
    ...
    def __init__(self,
        ...
        self.prob = prob
        self.labels_format = labels_format
        self.flip = Flip_arbitrary(dim=self.dim, labels_format=self.labels_format)
